...
cols = "\t"
# Original phenotype file sample name format (G - Grin name format, S - Soy1066 name format)
namform = "G"
...
df=pd.DataFrame(name_list,columns=["Samples"])
...
if phentyp=="Quant":
    ...
    for p in catM:
        df["Trait"]=df["Trait"].replace(p,2)
    df.loc[~df.Trait.isin([1,2]), 'Trait'] = 'NA'
